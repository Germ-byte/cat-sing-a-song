...
const renameTarget = ref<FavoritePlaylist | null>(null)
const deleteTarget = ref<FavoritePlaylist | null>(null)
...
function onCreateConfirm(name: string) {
...
// 重命名
function openRename(playlist: FavoritePlaylist) {
  renameTarget.value = playlist
  showRenameModal.value = true
}
function onRenameConfirm(name: string) {
  if (renameTarget.value && name.trim()) {
    favorites.renamePlaylist(renameTarget.value.id, name.trim())
  }
  renameTarget.value = null
}

// 删除
function openDelete(playlist: FavoritePlaylist) {
...
function onDeleteConfirm() {
  if (deleteTarget.value) {
    ...
    favorites.deletePlaylist(deleteTarget.value.id)
  }
...
<template>
  <div class="favorites-view">
      ...
      <div v-for="playlist in favorites.playlists" :key="playlist.id" class="card playlist-card">
        <div class="playlist-header" @click="toggleExpand(playlist.id)">
          ...
          <div class="playlist-actions" @click.stop>
            <button class="icon-btn icon-btn-sm" title="播放全部" :disabled="!playlist.songs.length" @click="playAll(playlist)">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
            </button>
            <button class="icon-btn icon-btn-sm" title="重命名" @click="openRename(playlist)">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" />
                <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" />
              </svg>
            </button>
            <button class="icon-btn icon-btn-sm del-btn" title="删除歌单" @click="openDelete(playlist)">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div v-if="expandedId === playlist.id" class="playlist-songs">
          <div v-if="playlist.songs.length" class="song-list">
            <div v-for="(song, i) in playlist.songs" :key="song.id" class="song-row" @click="playSong(song)">
              ...
              <button class="icon-btn icon-btn-sm del-btn song-del-btn" title="从歌单移除" @click.stop="removeSong(playlist, song)">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <ModalDialog
      ...
      mode="input"
      placeholder="请输入新名称"
      :default-value="renameTarget?.name || ''"
      confirm-text="保存"
...
.playlist-header:hover { background: var(--bg-hover); }
...
.playlist-name { font-size: 15px; font-weight: 600; color: var(--text-primary); }
.playlist-meta { font-size: 12px; color: var(--text-muted); margin-top: 3px; }

.playlist-actions { display: flex; align-items: center; gap: 4px; flex-shrink: 0; }
.del-btn:hover { color: #e06060; background: rgba(220, 80, 80, 0.12); }

.playlist-songs { border-top: 1px solid var(--border); padding: 4px 0; }
.playlist-empty { padding: 24px 16px; text-align: center; color: var(--text-muted); font-size: 13px; }

.song-list { padding: 4px 0; }
.song-del-btn { opacity: 0; transition: opacity 0.2s; color: var(--text-muted); }
.song-row:hover .song-del-btn { opacity: 1; }
</style>
